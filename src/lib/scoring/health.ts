...
export function buildHealthReport(
  ...
  const questions: Question[] = [
    profitabilityQuestion(f),
    growthQuestion(f, p, fundamentals),
    debtQuestion(f, altman, sector),
...
function profitabilityQuestion(f: Getter): Question {
  ...
  if (netIncome != null && margin != null) {
    ...
    if (netIncome > 0) {
      ...
      answer =
        ...
        `earning ${money(netIncome)} last year.`;
    } else {
      rating = "poor";
      answer =
        `No. It lost ${money(Math.abs(netIncome))} last year, ` +
    ...
    answer = netIncome > 0
      ? `Yes. It earned ${money(netIncome)} last year.`
      : `No. It lost ${money(Math.abs(netIncome))} last year.`;
  }
  ...
  return {
    ...
    metrics: [
      ...
      { label: "Operating cash flow", value: money(ocf), hint: "Actual cash generated by the business." },
    ],
  };
}

function growthQuestion(f: Getter, p: Getter, fundamentals: NormalizedFundamentals): Question {
  ...
  return {
    ...
    metrics: [
      ...
      { label: "Revenue", value: money(revNow), hint: "Total sales in the last reported year." },
    ],
  };
}
...
function debtQuestion(
  ...
  if (sector === "financial") {
    ...
    rating = "good";
    answer =
      `No — it holds more cash than debt, with ${money(Math.abs(netDebt))} left over ` +
  ...
  return {
    ...
    metrics: [
      ...
      {
        label: "Net debt",
        value: sector === "financial" ? "n/a for banks" : netDebt == null ? "—" : money(netDebt),
